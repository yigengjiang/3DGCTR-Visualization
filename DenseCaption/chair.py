...
def create_bounding_box(box_data, color):
    """
    Create a bounding box from the box data with the specified color.
    """
    bbox = o3d.geometry.OrientedBoundingBox.create_from_points(
        o3d.utility.Vector3dVector(box_data)
    )
    bbox.color = color
    # bbox.line_width = 10.0
    return bbox
...
if __name__ == "__main__":
    ...
    box_gt_data = np.load('/Users/leechina/Coding/3DGCTR-Visualization/DenseCaption/data/scene0496_gtboxes/scene0496_00-gt-4.npy')  # Replace with your actual file path
    box_dg_data = np.load('/Users/leechina/Coding/3DGCTR-Visualization/DenseCaption/data/scene0496_00-7.npy')  # Replace with your actual file path
    scene_data = np.load(scene_file_path)
    box_vc_data = (box_dg_data + box_gt_data) / 2  # Replace with your actual file path

    # Define the bounding box corner points
    bbox_vc_corners = o3d.utility.Vector3dVector(box_vc_data)
    ...
    edge_vc_color = [0, 0, 1]  # Blue
    edge_dg_color = [1, 0, 0]  # Red 
    edge_gt_color =  [0, 1, 0]  # Green

    
    # Create the bounding box edges with cylinders
    bbox_vc_edges = create_box_edges_with_cylinders(bbox_vc_corners, edge_radius, edge_vc_color)
    ...
    rgb_scene = scene_data[:, 3:6] / 255.0  # Normalize the RGB values to [0, 1]
    ...
    geometries = [scene_pcd] + bbox_dg_edges + bbox_gt_edges + bbox_vc_edges  # Add other geometries you want to visualize
