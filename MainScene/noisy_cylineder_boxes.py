...
def add_noise(boxes):
    np.random.seed(22)  # Set a fixed seed for reproducibility
    noise_parameters = []
    noisy_boxes = []
    # noisy_box = np.copy(box) # Create a copy of the box
    for _ in boxes:
        direction_index = np.random.choice([0, 1, 2])  # Randomly choose a direction (x, y, or z)
        shift_value = np.random.normal(0, 0.3)  # Generate a noise value for the shift
        noise_parameters.append((direction_index, shift_value))

    # Apply noise to each box
    for i, (box, (direction_index, shift_value)) in enumerate(zip(boxes, noise_parameters)):
        noisy_box = np.copy(box)
        noisy_box[:, direction_index] += shift_value
        noisy_boxes.append(noisy_box)

    return noisy_boxes
    

def visualize_both(scene, boxes, noise_boxes):
    geometries = []
    ...
    for index, box in enumerate(noise_boxes):
        box_corners = o3d.utility.Vector3dVector(box)
        edge_radius = 0.03

        edge_color = [0, 1, 0] # Red color
        # Generate a unique color for each set of box edges
        # edge_color = generate_unique_color(index, total_boxes)
...
if __name__ == "__main__":
    ...
    # visualize
    visualize_both(scene_pcd, boxes, noisy_boxes)
