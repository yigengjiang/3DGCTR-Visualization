...
def add_noise(box):
    ...
    shift_value = np.random.normal(0, 0.8)  # Generate a noise value for the shift
    noisy_box[:, direction_index] += shift_value  # Shift the entire copy of the box
...
if __name__ == "__main__":
    ...
    #print(box_gt_data)
    noise_box = add_noise(box_gt_data)
    #print(noise_box)

    if np.array_equal(box_gt_data, noise_box):
        print("equal")
    # exit()
    ...
    bbox_noise_corners = o3d.utility.Vector3dVector(noise_box)
    ...
    edge_noise_color = [0, 0, 1]  # Blue color
    ...
    bbox_noise_edges = create_box_edges_with_cylinders(bbox_noise_corners, edge_radius, edge_noise_color)
    ...
    geometries = [scene_pcd] + bbox_gt_edges + bbox_noise_edges   # Add other geometries you want to visualize
